...
Experiment.title = 'Fovea neglect effect for visual search in white noise -- Part 1';
Experiment.subject ='DF_t2';
Experiment.nTrials = 20;
Experiment.nSessions = 2;
Experiment.sessionLabels={'1','2'};
...
for S = 1:Experiment.nSessions
    for C = 1:Experiment.nConditions
        descending = maxAmp(C) + (minAmp(C) - maxAmp(C))/(Experiment.nLevels-1) * [0:(Experiment.nLevels-1)];
        Stimulus.targetAmplitude(S,C,:) = descending;
        for L = 1:Experiment.nLevels
            for T = 1:Experiment.nTrials
                if rand < 0.5
                    Stimulus.tLocation(T, S, C, L) = 0;
                else
                    Stimulus.tLocation(T, S, C, L) = 1;
                end
            end
        end
    end   
...
save(['p1_Experiment_', Experiment.subject, '.mat'], 'Experiment');
save(['p1_Stimulus_', Experiment.subject, '.mat'], 'Stimulus', '-v7.3');
...
save(['p1_Results_', Experiment.subject, '.mat'], 'Results');