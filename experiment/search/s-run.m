...
%% Mouse position
mousePos = zeros(Experiment.monitorPx(2), Experiment.monitorPx(1));
centerRing = round(sqrt((displayX-shiftedSpotCenters(1,1)).^2+(displayY-shiftedSpotCenters(1,2)).^2))...
        == floor(Stimulus.spotLength/2);
innerRing = round(sqrt((displayX-shiftedSpotCenters(1,1)).^2+(displayY-shiftedSpotCenters(1,2)).^2))...
    == floor(Stimulus.spotLength/2) - 1;
outerRing = round(sqrt((displayX-shiftedSpotCenters(1,1)).^2+(displayY-shiftedSpotCenters(1,2)).^2))...
    == floor(Stimulus.spotLength/2) + 1;
mousePos(centerRing | innerRing | outerRing) = 1;
...
for L = currentLevel:Experiment.nLevels
    if Experiment.expCompleted(S, C, L) == 0
        ...
        targetCoordX = [shiftedSpotCenters(1,1)-Stimulus.targetLength/2+1:shiftedSpotCenters(1,1)+Stimulus.targetLength/2];
        targetCoordY = [shiftedSpotCenters(1,2)-Stimulus.targetLength/2+1:shiftedSpotCenters(1,2)+Stimulus.targetLength/2];
        ...
        for T = 1:Experiment.nTrials
            ...
            [mouse_j, mouse_i] = find(mousePos==1); mouse_idx = randi(length(mouse_i));
            SetMouse(mouse_i(mouse_idx), mouse_j(mouse_idx), win);ShowCursor;
            ...
            if clicks == 1
                ...
                if whichButton == 1
                    valid_click = 0;
                    for sp = 1:Stimulus.nLocations-1
                        if sqrt((x-shiftedSpotCenters(sp,2)).^2+(y-shiftedSpotCenters(sp,1)).^2)<Stimulus.spotLength/2
                            valid_click = 1;
                            Results.hResponse(T,S,C,L) = sp;
                            break;
                        end
                    end
        ...
        save(['p3_Experiment_', Experiment.subject, '.mat'], 'Experiment');
        save(['p3_Results_', Experiment.subject, '.mat'], 'Results');
    end
end
...
if S==1 && L==1 && T==1
    ...
    save('p3_stimulus.mat', "texture_uniform_luminance_gc", "texture_circular_cue_gc", "stimulus_gc", "background_gc");
end

Stimulus_no_background = rmfield(Stimulus, 'scene');
save(['p3_vars_Results_',Experiment.subject,'.mat'], '-struct', 'Results');
save(['p3_vars_Experiments_',Experiment.subject,'.mat'], '-struct', 'Experiment');
save(['p3_vars_Stimulus_',Experiment.subject,'.mat'], '-struct', 'Stimulus_no_background');
