...
Experiment.title = 'Fovea neglect effect for visual search in white noise -- Part 4';
Experiment.subject ='DF_t1';
Experiment.nTrials = 30;
Experiment.nSessions = 4;
Experiment.sessionLabels = {'1','2','3', '4'};
Experiment.nConditions = 1;
Experiment.conditionLabels = {'white noise'};
Experiment.nLevels = 10;
...
Stimulus.locations = [0];
for i =1:size(Stimulus.spotCenters,1)
    if sqrt((Stimulus.spotCenters(i,1)-Stimulus.spotCenters(1,1)).^2+(Stimulus.spotCenters(i,2)-Stimulus.spotCenters(1,2)).^2)<Stimulus.spotDistance+Stimulus.spotLength/2
        Stimulus.locations(end+1) = i;
    end
end
...
for S = 1:Experiment.nSessions
    for C = 1:Experiment.nConditions
        for L = 1:Experiment.nLevels
            Stimulus.targetAmplitude(S,C,L) = 3.8;
            for T = 1:Experiment.nTrials
                if rand < 0.5
                    Stimulus.tLocation(T, S, C, L) = 0;
                else
                    Stimulus.tLocation(T, S, C, L) =  randsample(Stimulus.locations(2:end),1);
                end
            end
        end
    end   
...
save(['p4_Experiment_', Experiment.subject, '.mat'], 'Experiment');
save(['p4_Stimulus_', Experiment.subject, '.mat'], 'Stimulus', '-v7.3');
...
save(['p4_Results_', Experiment.subject, '.mat'], 'Results');